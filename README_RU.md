...
Конструкторы источников в памяти не копируют и не преобразуют переданный срез. Валидация конкретной операции может декодировать PEM или Base64 до нативного вызова. На Linux и Windows length-delimited входы в памяти передаются в KalkanCrypt напрямую, без копирования в адаптере. `File` передаёт исходный путь после проверки на пустое значение и NUL; нативный адаптер добавляет C-string терминатор, необходимый для `KC_IN_FILE`. Не изменяйте заимствованные срезы и файлы до завершения вызова.
