...
- `WithBufferSize` задаёт общую начальную ёмкость для вызовов с выходным буфером, которым передаётся его размер. Поля ёмкости в запросе имеют приоритет над этой опцией. Без неё используются начальные размеры 128 байт для хешей, 4 KiB для метаданных, 8 KiB для сертификатов и 64 KiB для подписей и остальных выходных буферов. Перед вызовом ёмкость записывается в выходной параметр длины. При `KCR_BUFFER_TOO_SMALL` обёртка увеличивает буфер и повторяет вызов.
- `WithMaxBufferSize` ограничивает начальные аллокации и рост буферов при повторных попытках; значение по умолчанию равно 64 MiB. Для `KC_GetTokens` и `KC_GetCertificatesList` опция ограничивает только аллокацию: SDK по-прежнему не получает лимит в байтах.

Положительные значения этих опций нормализуются до значения не менее 64 KiB.
...
`WithMaxInputSize` ограничивает входы в памяти на уровне корневого API. Ограничение не распространяется на файловые источники и нативные выходные буферы. `WithMaxOutputBufferSize` ограничивает начальную аллокацию и рост буфера при повторных попытках и передаётся в `ckalkan.WithMaxBufferSize`.
