...
func TestZipConVerifyFixtures(t *testing.T) {
	...
	client := newIntegrationClient(t, largeBufferOptions()...)
	loadCertificates(t, client, assets)

	for _, zipPath := range assets.ZIPs {
		t.Run(filepath.Base(zipPath), func(t *testing.T) {
			info, err := client.ZipConVerify(testfixture.CopyZIP(t, zipPath), ckalkan.NoCheckCertTime)
			if err != nil {
...
func TestGetCertFromZipFileFixtures(t *testing.T) {
	...
	for _, zipPath := range assets.ZIPs {
		t.Run(filepath.Base(zipPath), func(t *testing.T) {
			cert, err := client.GetCertFromZipFile(testfixture.CopyZIP(t, zipPath), ckalkan.NoCheckCertTime, 0)
			if err != nil {
