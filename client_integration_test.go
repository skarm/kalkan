...
func assertHashSigning(t *testing.T, ctx context.Context, client *Client, payload []byte) {
	...
	if err != nil || verification == nil {
		t.Fatalf("VerifyCMS(SignHash original payload) = %#v, %v, want successful positive control", verification, err)
	}
	requireContains(t, "SignHash CMS verification", verification.Info, "Verify - OK")

	tampered := bytes.Clone(payload)
	tampered[0] ^= 1
	request.Data = Bytes(tampered)
	...
	requireNativeErrorCode(t, "VerifyCMS(SignHash tampered payload)", err, cmsDigestVerificationFailure)
}
